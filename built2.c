...
void add_keyy(vars_t *vars)
{
        ...
        for (i = 0; vars->env[i] != NULL; i++)
                newenv[i] = vars->env[i];
        newenv[i] = add_valuee(vars->av[1], vars->av[2]);
        if (newenv[i] == NULL)
        {
                print_errorr(vars, NULL);
                free(vars->buffer);
...
/**
 * find_keyy - finds an environment variable
...
/**
 * add_valuee - create a new environment variable string
 * @key: variable name
...
/**
 * _atoii - converts a string into an integer
