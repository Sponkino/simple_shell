...
int path_executee(char *command, vars_t *vars)
{
	...
	{
		print_errorr(vars, ": Permission denied\n");
		vars->status = 126;
	}
	return (0);
}
...
/**
 * find_pathh - finds the PATH variable
...
/**
 * check_for_pathh - checks if the command is in the PATH
 * @vars: variables
 *
 * Return: void
 */
void check_for_pathh(vars_t *vars)
{
	...
	if (check_for_dirr(vars->av[0]))
		r = execute_cwdd(vars);
	else
	{
		path = find_pathh(vars->env);
		if (path != NULL)
		{
			path_dup = _strdupp(path + 5);
			path_tokens = token(path_dup, ":");
			for (i = 0; path_tokens && path_tokens[i]; i++, free(check))
			{
				check = _strcatt(path_tokens[i], vars->av[0]);
				if (stat(check, &buf) == 0)
				{
					r = path_executee(check, vars);
					free(check);
					break;
				}
			}
...
/**
 * execute_cwdd - executes the command in the current working directory
 * @vars: pointer to struct of variables
 *
...
/**
 * check_for_dirr - checks if the command is a part of a path
 * @str: command
 *
